...
scores = cross_validation.cross_val_score(alg, titanic[predictors], titanic['Survived'], cv=kf)scores.mean()
...
def get_title(name):
    ...
    return ''titles = titanic['Name'].apply(get_title)
...
scores = -np.log10(selector.pvalues_)plt.bar(range(len(predictors)), scores)
